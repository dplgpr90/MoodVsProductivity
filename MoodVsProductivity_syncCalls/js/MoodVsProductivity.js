...
function analyze () {	
	...
	var errorCollaborators = 0;
	// Gets an array of structures containing nickname, real name, GitHub url and Twitter url (now empty) for each collaborator
	var collaboratorsInfo = getCollaboratorsInfo();
	// Sets width of an increment of progress bar
	// setProgressBarSteps(collaboratorsInfo.length);
	// Creates activity bubbles for every commit
	var activityBubbles = getActivityBubbles();
	// Result array
	var collaboratorsAnalysisResult = new Array();
	// Makes analysis for each collaborator
	$.each(collaboratorsInfo, function() {	
		if(this != undefined){
			// Increases progress bar width
			// increaseProgressBar();		
			/*
				ANALYSIS STEPS:
			...
			*/
			...
			var computedActivityBubbles = getActivityBubblesByCollaborator(activityBubbles, this);
	...
	// Returns result
	return {"collaboratorsAnalysisResult": collaboratorsAnalysisResult, "errorCollaborators": errorCollaborators};
}

// Returns an array of structures containing nickname, real name, GitHub url and Twitter url (now empty) for each collaborator
...
function getCollaboratorInfo(collaborator){
	...
	$.ajax({
	  	...
	  	success: function(data){
	  		...
	  		collaboratorInfo = {"login": data.login, "name": data.name, "GitHubUrl": undefined, "TwitterUrl": undefined};
	  	},
		error: function (jqXHR, textStatus, errorThrown){
...
// Returns activity bubbles with productivityIndex computed
function getBubblesProductivityIndex (activityBubbles) {
	activityBubbles = checkFilesActivityBubbles(activityBubbles);
	activityBubbles = checkBuggyActivityBubbles(activityBubbles);
...
// Returns activity bubbles with numbers of files added, modified and removed according to the commit linket the each bubble
function checkFilesActivityBubbles(activityBubbles){
...
// Returns activity bubbles after checking in which one buggy code has been written
function checkBuggyActivityBubbles(activityBubbles){
	$.each(activityBubbles, function() {
				...
				$.ajax({
			  		...
			  		success: function(data){	
						...
						/* 
			  			...
			  			*/	
						activityBubbles = findModifiedFilesBuggy(activityBubbles, activityBubbles.indexOf(this_bubble), modifiedFiles);
					},
					error: function (jqXHR, textStatus, errorThrown){
...
function getActivityBubblesByCollaborator(activityBubbles, collaborator){
...
function getProductivityIndexMergingActivityBubbles(activityBubbles){
	// Following two conditions are for completness (they can be deleted in this implementation)
	if(activityBubbles.length == 0){
		return undefined;
	}
	if(activityBubbles.length == 1){
		return activityBubbles[0].productivityIndex;
	}
	var productivityIndex = 0;
	$.each(activityBubbles, function(){
	...
	// Computes average
	productivityIndex = productivityIndex/activityBubbles.length;
	return productivityIndex;
}
